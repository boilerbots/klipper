...
class HomingMove:
    ...
    def homing_move(self, movepos, speed, probe_pos=False,
        ...
        self.stepper_positions = { s.get_name(): StepperPosition(s, name)
        ...
        self.toolhead.flush_step_generation()
        for sp in self.stepper_positions.values():
            tt = trigger_times.get(sp.endstop_name, move_end_print_time)
            sp.note_home_end(tt)
        if probe_pos:
            halt_steps = {sname: sp.halt_pos - sp.start_pos
                          for sname, sp in self.stepper_positions.items()}
            trig_steps = {sname: sp.trig_pos - sp.start_pos
                          for sname, sp in self.stepper_positions.items()}
            ...
            over_steps = {sname: sp.halt_pos - sp.trig_pos
                          for sname, sp in self.stepper_positions.items()}
...
class Homing:
    ...
    def home_rails(self, rails, forcepos, movepos):
        ...
        self.toolhead.flush_step_generation()
        sps = hmove.stepper_positions
        self.trigger_mcu_pos = {sname: sp.trig_pos for sname, sp in sps.items()}
        self.adjust_pos = {}
